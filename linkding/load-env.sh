...
# Load the active Hermes .env (or an explicit override path)
# Usage: load_env_file [/optional/override/path]
load_env_file() {
    local hermes_home="${HERMES_HOME:-/opt/data}"
    local env_file="${1:-$hermes_home/.env}"

    if [[ ! -f "$env_file" ]]; then
        echo "ERROR: $env_file not found" >&2
        echo "Add LINKDING_URL and LINKDING_API_KEY to the active Hermes .env" >&2
        return 1
    fi
