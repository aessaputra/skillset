...
source "$_LOAD_ENV" || { echo "ERROR: bundled load-env.sh not found" >&2; exit 1; }

# Load credentials only for commands that call the API.
case "${1:-}" in
    -h|--help|help|"") ;;
    *) load_service_credentials "linkding" "LINKDING_URL" "LINKDING_API_KEY" ;;
esac

# Remove trailing slash when credentials were loaded.
